    ...
    <div className="min-h-screen bg-[#F8FAFC] pb-20 font-sans">
      ...
      <main className="max-w-2xl mx-auto p-4 space-y-6">
        ...
        <div className="rounded-[2.5rem] overflow-hidden shadow-xl shadow-slate-200 border-4 border-white h-64 relative">
          ...
          <div className="absolute bottom-4 left-4 bg-white/90 backdrop-blur-sm px-4 py-2 rounded-2xl shadow-lg border border-white">
            <div className="flex items-center gap-2 text-emerald-600">
              <MapPin size={16} />
              <span className="text-xs font-bold uppercase tracking-widest">Verified Spot</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-slate-100 flex justify-between items-center">
          <div className="space-y-1">
            <div className="flex items-center gap-2 text-slate-400">
              <Calendar size={14} />
              <span className="text-[10px] font-black uppercase tracking-widest">Logged On</span>
            </div>
        ...
        <div className="space-y-4">
          ...
          {/* MEETING SECTION */}
          {activity.meeting && (
            <DataSection title="Meeting Intelligence" icon={<Briefcase size={18} className="text-blue-500" />}>
              <GridItem label="Primary Contact" value={activity.meeting.personName} icon={<User size={14} />} />
              <GridItem label="Category" value={activity.meeting.category} icon={<Tag size={14} />} />
              <GridItem label="Village" value={activity.meeting.village || "Not Specified"} icon={<MapPin size={14} />} />
              <GridItem 
          ...
          {/* SAMPLE SECTION */}
          {activity.sample && (
            <DataSection title="Sample Distribution" icon={<Package size={18} className="text-amber-500" />}>
              <GridItem label="Product Name" value={activity.sample.productName} />
              <GridItem label="Quantity Issued" value={activity.sample.quantity} />
              <div className="col-span-2 mt-2 p-3 bg-amber-50 rounded-xl text-xs text-amber-800 font-medium">
                <strong>Purpose:</strong> {activity.sample.purpose}
              </div>
            </DataSection>
          )}
          ...
          {/* SALE SECTION */}
          {activity.sale && (
            <DataSection title="Transaction Details" icon={<ShoppingCart size={18} className="text-emerald-500" />}>
              <GridItem label="Customer" value={activity.sale.customerName} />
              <GridItem label="Mode" value={activity.sale.mode} />
              <GridItem label="Product ID" value={activity.sale.productId} />
              <GridItem label="Volume" value={activity.sale.quantity} />
              <div className="col-span-2 flex gap-2 pt-2">
                <Badge label="Repeat Order" active={activity.sale.isRepeatOrder} />
                <Badge label="Follow-up Sale" active={activity.sale.isFollowUpSale} />
              </div>
             ...
             <div className="space-y-4">
                {activity.address && (
                  <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                    <p className="text-[10px] font-black text-slate-400 uppercase mb-1">Geocoded Address</p>
