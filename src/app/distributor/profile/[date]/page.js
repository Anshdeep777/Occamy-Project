    ...
    <div className="min-h-screen bg-[#fcfdfe] pb-20 font-sans selection:bg-emerald-100 mb-16">
      <div className="max-w-md mx-auto px-5 pt-8 space-y-6">
        ...
        <header className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black tracking-tight text-slate-900 leading-tight">Day Summary</h1>
        ...
        <div className="rounded-[2.5rem] overflow-hidden shadow-xl border-4 border-white h-48 bg-slate-100">
          {activities.length > 0 ? (
            ...
            <div className="h-full flex items-center justify-center text-slate-400 text-xs font-bold">No Route Data</div>
          )}
        </div>

        {/* STATS GRID */}
        <div className="grid grid-cols-2 gap-3">
          <Stat label="Meetings" value={data?.meetings || 0} icon={<Users size={14} />} color="text-indigo-600" bg="bg-indigo-50" />
          <Stat label="Samples" value={data?.samples || 0} icon={<Package size={14} />} color="text-orange-600" bg="bg-orange-50" />
          <Stat label="Sales" value={data?.sales || 0} icon={<ShoppingBag size={14} />} color="text-emerald-600" bg="bg-emerald-50" />
          <Stat label="Total Logs" value={activities.length} icon={<ClipboardList size={14} />} color="text-slate-600" bg="bg-slate-50" />
        </div>

        {/* TIMELINE SECTION */}
        <div className="space-y-4">
          <div className="flex items-center justify-between px-2">
            <h2 className="text-[12px] font-black text-slate-400 uppercase tracking-[0.2em]">Timeline</h2>
            <span className="text-[9px] font-bold bg-slate-100 px-2 py-0.5 rounded text-slate-500">{activities.length} Entries</span>
          </div>
          
          {activities.length === 0 ? (
            <div className={glassCard + " p-10 text-center"}>
              <p className="text-xs font-bold text-slate-400">No activity recorded for this date</p>
            </div>
          ) : (
                ...
                >
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <div className="flex items-center gap-2">
                       <span className="text-[9px] font-black text-emerald-600 uppercase opacity-0 group-hover:opacity-100 transition-opacity">Details</span>
