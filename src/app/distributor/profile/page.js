  ...
  if (!profile) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#F2F2F7] gap-4">
        ...
        <p className="text-slate-400 font-black text-xs uppercase tracking-[0.2em]">Authenticating...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#F2F2F7] pb-40 pt-10 px-5 space-y-10 font-sans selection:bg-emerald-100">
      ...
      <div className="flex justify-between items-end px-2">
        <div>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Distributor Portal</p>
            <h1 className="text-4xl font-extrabold tracking-tighter text-slate-900">Account.</h1>
        </div>
        ...
        <div className="mt-8 pt-6 border-t border-slate-200/50 space-y-2">
            <div className="flex items-center justify-between p-4 bg-white/50 hover:bg-white rounded-2xl transition-all cursor-pointer group border border-transparent hover:border-emerald-100 shadow-sm">
                <div className="flex items-center gap-4">
                    ...
                    <span className="text-[15px] font-extrabold text-slate-800">Personal Information</span>
                </div>
      ...
      <section>
        ...
        {stats ? (
          ...
          <div className="h-40 bg-white/40 rounded-[2.5rem] animate-pulse flex items-center justify-center border border-white/20">
              <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Calculating Metrics...</p>
          </div>
        )}
      </section>
      ...
      <section className="space-y-8">
        <div>
            <p className={sectionLabel}>Attendance Tracking</p>
            <div className="bg-emerald-600 text-white p-4 rounded-3xl mb-4 flex items-center gap-3 shadow-lg shadow-emerald-200">
                ...
                <p className="text-[11px] font-bold leading-tight uppercase tracking-wider">
                    Tap any day in the calendar to audit detailed field activity logs.
                </p>
            </div>
            <CalendarView />
        </div>
        ...
        <div>
            <p className={sectionLabel}>Financial Insights</p>
            <MonthlySummary />
        </div>
      </section>
      ...
      <div className="px-2 pt-4">
          <button className="w-full py-5 rounded-[2rem] bg-rose-50 text-rose-600 font-black text-xs uppercase tracking-[0.2em] active:scale-[0.98] transition-all border border-rose-100 shadow-sm">
              Sign Out Account
          </button>
      </div>
