...
export default function LogActivityPage() {
  ...
  const potentialLabels = [
    { label: '5–10kg', desc: 'Emerging' },
    { label: '20–50kg', desc: 'Growth' },
    { label: '100–200kg', desc: 'High' },
    { label: '200kg+', desc: 'Enterprise' }
  ];
    ...
    <div className="min-h-screen bg-[#F2F2F7] bg-gradient-to-b from-[#F2F2F7] to-[#FFFFFF] font-sans pb-32">
      ...
      <div className="max-w-md mx-auto px-5 pt-12 space-y-6">
        ...
        <header className="px-2 space-y-1">
          ...
          <p className="text-slate-500 font-bold text-sm">Activity Details</p>
        </header>
        ...
        {type.startsWith("MEETING") && (
          <>
              ...
              {['ONE_ON_ONE', 'GROUP'].map((kind) => (
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl text-[13px] font-black transition-all duration-300 ${
                    meeting.meetingKind === kind ? 'bg-white shadow-xl text-slate-900 scale-[1.02]' : 'text-slate-500 hover:text-slate-700'
                  }`}
                >
            ...
            <div className={glassCard + " space-y-5"}>
              <div>
                <label className={iosLabel}>Primary Contact</label>
                <input
              ...
              <div className="grid grid-cols-2 gap-4">
                <select className={iosInput} value={meeting.category} onChange={(e) => setMeeting({ ...meeting, category: e.target.value })}>
                    ...
                    <option value="INFLUENCER">Agent</option>
                </select>
                <input placeholder="Phone" className={iosInput} value={meeting.contact} onChange={(e) => setMeeting({ ...meeting, contact: e.target.value })} />
              </div>
            </div>
            ...
            <div className={glassCard + " space-y-6"}>
              <div className="flex justify-between items-center">
                <label className="text-[14px] font-black text-slate-900 flex items-center gap-2 uppercase tracking-tight">
                  <Sparkles size={18} className="text-amber-500" /> Potential
                </label>
                <div className="px-3 py-1 bg-emerald-500 text-white text-[10px] font-black rounded-lg uppercase">
                  {potentialLabels[meeting.businessPotential - 1]?.desc}
                </div>
              </div>
              <input
        ...
        {type === "SALE" && (
          <div className={glassCard + " space-y-5"}>
            <label className={iosLabel}>Sale Record</label>
        ...
        {/* ================= NOTES & ODOMETER ================= */}
        <div className={glassCard + " space-y-5"}>
          <label className={iosLabel}>Additional Info</label>
          <textarea
            placeholder="Notes..."
          ...
          <div className="relative">
            ...
            <input type="number" placeholder="Odometer" className={iosInput + " pl-12"} value={odometer} onChange={(e) => setOdometer(e.target.value)} />
          </div>
        </div>
