...
function LogActivityContent() {
  ...
  const [geo, setGeo] = useState({ state: "Detecting...", district: "Detecting...", village: "" });
      ...
      <div className="max-w-md mx-auto px-5 pt-12 space-y-6">
        <header className="px-2 flex justify-between items-end">
          <div>
            ...
            <p className="text-slate-500 font-bold text-sm">Real-time Field Entry</p>
          </div>
          <div className="bg-emerald-500 px-4 py-2 rounded-2xl shadow-lg shadow-emerald-200 text-white text-right">
             <div className="flex items-center gap-1.5 justify-end">
                <Clock size={12} className="animate-pulse" />
                <span className="text-[10px] font-black uppercase leading-none">Current time</span>
             </div>
        ...
        <div className="bg-emerald-600 rounded-[2.5rem] p-6 text-white shadow-2xl shadow-emerald-200 relative overflow-hidden group">
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-2 mb-4">
              ...
              <div>
                <h3 className="text-[10px] font-black uppercase opacity-80 tracking-widest">Verified Location</h3>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4 border-t border-white/20 pt-4">
               <div>
                 <p className="text-[10px] font-black opacity-60 uppercase">District</p>
                 <p className="font-bold text-xs truncate">{geo.district}</p>
               </div>
               <div>
                 <p className="text-[10px] font-black opacity-60 uppercase">Coordinates</p>
        ...
        {type.startsWith("MEETING") && (
          <>
            ...
            <div className={glassCard + " space-y-5"}>
              <input placeholder="Person / Group Name" className={iosInput} value={meeting.personName} onChange={(e) => setMeeting({ ...meeting, personName: e.target.value })} />
              <div className="grid grid-cols-2 gap-4">
                ...
                <input placeholder="Contact Phone" className={iosInput} value={meeting.contact} onChange={(e) => setMeeting({ ...meeting, contact: e.target.value })} />
              </div>
              {meeting.meetingKind === "GROUP" && (
                 <input type="number" placeholder="Attendee Count" className={iosInput} value={meeting.attendeeCount} onChange={(e) => setMeeting({ ...meeting, attendeeCount: e.target.value })} />
              )}
              <select className={iosInput} value={meeting.intent} onChange={(e) => setMeeting({ ...meeting, intent: e.target.value })}>
                  <option value="TRIAL">Trial Intent</option>
        ...
        {type === "SAMPLE_DISTRIBUTION" && (
          <div className={glassCard + " space-y-5"}>
            <label className={iosLabel}>Sample Details</label>
            ...
            <div className="grid grid-cols-2 gap-4">
                <input type="number" placeholder="Qty" className={iosInput} value={sample.quantity} onChange={(e) => setSample({ ...sample, quantity: e.target.value })} />
                <select className={iosInput} value={sample.purpose} onChange={(e) => setSample({ ...sample, purpose: e.target.value })}>
                    <option value="TRIAL">Trial</option>
                    <option value="DEMO">Demo</option>
                </select>
            </div>
            <input placeholder="Given To (Name)" className={iosInput} value={sample.givenTo} onChange={(e) => setSample({ ...sample, givenTo: e.target.value })} />
          </div>
        )}

        {type === "SALE" && (
          <div className={glassCard + " space-y-5"}>
            <label className={iosLabel}>Commercial Sale Record</label>
            <input placeholder="Customer Name" className={iosInput} value={sale.customerName} onChange={(e) => setSale({ ...sale, customerName: e.target.value })} />
            <div className="grid grid-cols-2 gap-4">
            ...
            <div className="grid grid-cols-2 gap-4">
                <input type="number" placeholder="Quantity" className={iosInput} value={sale.quantity} onChange={(e) => setSale({ ...sale, quantity: e.target.value })} />
                <input type="number" placeholder="Total Amount" className={iosInput} value={sale.amount} onChange={(e) => setSale({ ...sale, amount: e.target.value })} />
            </div>
            <div className="flex gap-2">
                <button onClick={() => setSale({...sale, isRepeatOrder: !sale.isRepeatOrder})} className={`flex-1 py-3 rounded-xl text-[10px] font-black border ${sale.isRepeatOrder ? 'bg-emerald-500 text-white' : 'bg-white text-slate-400'}`}>REPEAT</button>
                <button onClick={() => setSale({...sale, isFollowUpSale: !sale.isFollowUpSale})} className={`flex-1 py-3 rounded-xl text-[10px] font-black border ${sale.isFollowUpSale ? 'bg-blue-500 text-white' : 'bg-white text-slate-400'}`}>FOLLOW-UP</button>
            </div>
          </div>
        )}

        <div className={glassCard}>
          <label className={iosLabel}>Photos & Proof</label>
          <div className="flex gap-3 overflow-x-auto pb-2">
            <div className="relative shrink-0 w-24 h-24 bg-slate-100 rounded-3xl border-2 border-dashed border-slate-300 flex flex-col items-center justify-center text-slate-400">
                ...
                <Camera size={24} />
                <span className="text-[10px] font-black mt-1 uppercase">Add</span>
            </div>
        ...
        <div className={glassCard + " space-y-5"}>
          ...
          <div className="relative">
            ...
            <input type="number" placeholder="Odometer Reading" className={iosInput + " pl-12"} value={odometer} onChange={(e) => setOdometer(e.target.value)} />
          </div>
        </div>
        ...
        >
          {isSubmitting ? <Loader2 className="animate-spin" size={20} /> : "FINISH ACTIVITY"}
